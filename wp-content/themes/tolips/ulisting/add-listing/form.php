...
<div id="stm-listing-form-listing">
    ...
    <div class="step" v-if="step == 'form'">
        ...
        <div class="form-listing-item listing-form-select-plan">
                ...
                <div v-for="plan in user_plans" class="stm-col-xl-3 stm-col-lg-4 stm-col-md-3 stm-col-6">
                    <div class="card card-plan" v-if="plan.type != 'feature' && (plan.listing_limit || plan.listing_limit == 0) || (listing_plan_select == 'none') && (plan.id == 'free' || plan.id == listing_plan_select) && plan.status === 'active'">
                        <div class="card-body">
                            ...
                            <a v-if="!listing_plan_one_time" :disabled="plan.listing_limit <= plan.use_listing_limit" @click="select_limit_plan(plan)" class="btn-inline btn-select-plan"></a>
                        </div>
                    </div>
                </div>
            </div>
            <div class="stm-row">
                ...
                <template v-else>
                    <p class="col text-center" style="padding: 5px 0"><?php echo __('Admin does not need to select a plan', 'ulisting')?></p>
                </template>
            </div>
    ...
    <div class="step"  v-if="step == 'last'">
        <div>
            <label><?php esc_html_e('Plan for Featured Listings', "tolips")?></label>
            <div class="stm-row">
                ...
                <div class="col text-center" v-if="!planAccess">
                    <a href="<?php echo ulisting_get_page_link( 'pricing_plan' )?>"><?php echo esc_html__('Buy plan', "tolips")?> </a>
                </div>
            </div>
            <span v-if="errors['user_plan']" style="color: red">{{errors['user_plan']}}</span>
        </div>
        <div>
            ...
            <span v-if="loading"><?php echo esc_html__("Loading...", "tolips") ?></span>
            <button class="btn-theme btn-medium" v-if="!loading" @click="set_feature">
                <?php echo esc_html__("Set feature", "tolips") ?>
            </button>
            <a href="<?php echo \uListing\Classes\StmUser::getUrl('my-listing')?>" class="btn-theme btn-medium">
                <?php echo esc_html__("My listing", "tolips") ?>
            </a>
        </div>
    </div>
