...
if($is_google){
    ...
    wp_enqueue_script('google-maps', $protocol . "://maps.googleapis.com/maps/api/js?libraries=geometry,places&key=".get_option('google_api_key')."&callback=googleApiLoadw", array(), '', true);
}
?>
...
<?php if($model):?>
    <?php if($is_google): ?>
        ...
        <div>
            ...
            <?php endif;?>
            <span class="stm-ulisting-location-field-wrapper">
    ...
    <?php else:?>
        <stm-field-osm-location inline-template
            ...
            <div class="inventory-location-filter">
                <div class="inventory-location-field stm-ulisting-location-field-wrapper">
                    <input class="form-control" data-v-bind_id="id" data-v-bind_placeholder='placeholder' data-v-model='value.address' type='text'>
                    <span @click="findMyLocation"  class="stm-find-my-location"></span>
                </div>
            </div>
