...
$list = '<ul data-v-if="view_type==\'list\'" class="list-inline">
			<li class="list-inline-item" data-v-for=" (item, key)  in listing_order">
				<span style="cursor: pointer" data-v-on_click="selected=item; change()">{{item.label}}</span>
			</li>
		 </ul>';

$select = '<ulisting-select2 v-if="selected" data-v-bind_options="listing_order" v-model="selected" text="label" theme=""></ulisting-select2>';

$sort_panel = '<div class="listing-sort"><i class="icon las la-sort"></i><div '.\uListing\Classes\Builder\UListingBuilder::generation_html_attribute($element).' >
